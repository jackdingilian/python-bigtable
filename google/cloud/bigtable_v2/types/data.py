...
class PartialResultSet(proto.Message):
    ...
    -  a place to keep track of the most recent ``resume_token`` for
       ...
       ``p.proto_rows_batch.bytes`` to ``buffer`` } if p.batch_complete
       and ``buffer`` is not empty { parse ``buffer`` as ``ProtoRows``
       message, clearing ``buffer`` add parsed rows to end of ``queue``
       } if p.restart_token { release results in ``queue`` save
    ...
    Attributes:
        ...
        batch_checksum (int):
            ...
            true. In other words, clients may assume that a batch will
            never cross a ``result_token`` boundary.
        ...
        estimated_batch_size (int):
            ...
            stream, on the first message after a ``batch_complete``

...
class _Reader(ABC, Generic[T]):
    ...
    @abstractmethod
    def consume(self, batches_to_consume: List[bytes]) -> Optional[Iterable[T]]:
        """TODO This method receives a parsable chunk of bytes and returns either a None if there is
...
class _QueryResultRowReader(_Reader[QueryResultRow]):
    ...
    def consume(
        self, batches_to_consume: List[bytes], metadata: ProtoMetadata

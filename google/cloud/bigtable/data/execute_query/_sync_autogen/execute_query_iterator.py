...
def _should_retry(retryable_excs):
    ...
    def handling_prepare_expiry_predicate(exception: Exception) -> bool:
        return is_prepared_query_expired_exception(exception) or base_predicate(
            exception
        )
...
class ExecuteQueryIterator:
    def __init__(
        ...
        **Please Note** this is not meant to be constructed directly by applications. It should always
        ...
        self._instance_name = self._client._gapic_client.instance_path(
            self._client.project, self._instance_id
        )
    ...
    def _get_latest_prepare_response(self):
        if not self._latest_prepare_version:
            self._latest_prepare_version = self._prepared_statement._get_latest_plan()
        return self._latest_prepare_version

    def _trigger_plan_update_on_expiry(self):
        expired_version = self._latest_prepare_version.version
        self._latest_prepare_version = self._prepared_statement._mark_expired(
            expired_version
        )
    ...
    def _make_request_with_resume_token(self):
        ...
        latest_prepare_version = self._get_latest_prepare_response()
        (complete, timed_out) = CrossSync._Sync_Impl.wait(
            [latest_prepare_version.prepare_response], timeout=attempt_timeout_seconds
        )
        if timed_out:
            raise DeadlineExceeded()
        elif not complete:
            raise ValueError(
                "Unexpected failure waiting for PreparedStatement to refresh"
            )
        ...
        try:
            for res in self._client._gapic_client.execute_query(
                request,
            ...
            if is_prepared_query_expired_exception(e):
                self._trigger_plan_update_on_expiry()
            raise e
